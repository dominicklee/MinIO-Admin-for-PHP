...
class MinIO {
	...
	*/
	...
	function addKeys($endpoint, $accessKey, $secret)
	{
		...
		$output = shell_exec("../min/mc config host add {$this->alias} {$this->endpoint} {$this->accessKey} {$this->secret} 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin info {$this->alias} --json 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin service restart {$this->alias} 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	function stopServer()
	{ 
		$output = shell_exec("../min/mc admin service stop {$this->alias} 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin user remove {$this->alias}/ {$accessKey} 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin user info {$this->alias} {$accessKey} 2>&1"); 
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin policy set {$this->alias}/ {$policy} user={$accessKey} 2>&1");
		echo $output;
	} 
	...
	*/
	...
	{ 
		$output = shell_exec("../min/mc admin policy list {$this->alias}/ --json 2>&1");
		echo $output;
	} 
	...
	*/
	...
	{ 
		// Example: ../min/mc admin policy add myminio/ userbucketsonly /tmp/userbucketsonly.json
		$filename = basename($policyFile);         // $file is set to "index.json"
		$policyName = basename($filename, ".json"); // $file is set to "index"

		$output = shell_exec("../min/mc admin policy add {$this->alias}/ {$policyName} {$policyFile} 2>&1");
		echo $output;
	}
	...
	*/
	...
	{ 
		// Example:  ../min/mc admin policy remove myminio/ userbucketsonly
		$output = shell_exec("../min/mc admin policy remove {$this->alias}/ {$policyName} 2>&1");
		echo $output;
	}
